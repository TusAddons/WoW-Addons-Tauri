...
function window:CreateFrame1()
	...
	--> window controls
		...
		--config disable spam (custom injected)
			g:NewLabel (frame1, _, "$parentDisableSpamLabel", "DisableSpamLabel", "Desactivar Spam", "GameFontHighlightLeft")
			g:NewSwitch (frame1, _, "$parentDisableSpamSwitch", "DisableSpamSwitch", 60, 20, _, _, _detalhes_global and _detalhes_global.disable_update_spam, nil, nil, nil, nil, options_switch_template)
			frame1.DisableSpamSwitch:SetPoint("left", frame1.DisableSpamLabel, "right", 2, 0)
			frame1.DisableSpamSwitch.OnSwitch = function (_, _, value)
				if not _detalhes_global then _detalhes_global = {} end
				_detalhes_global.disable_update_spam = value
			end
			frame1.DisableSpamSwitch:SetAsCheckBox()
			window:CreateLineBackground2 (frame1, "DisableSpamSwitch", "DisableSpamLabel", "Desactiva los mensajes automáticos que se envían a los demás pidiéndoles que actualicen.")
