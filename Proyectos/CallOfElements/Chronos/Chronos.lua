...
--	$Date: 2008-08-28 15:47:18 $
--	$Rev: 4467 $
--	
--	2010-09-16 (Kiki):
--	 - Updated for cataclysm
--	 - Updated Version number to 2.17
--	2008-08-28 (Kiki):
--	 - Updated for WotLK
--	 - Updated Version number to 2.15
--	2008-06-06 (Kiki):
--	 - Now removing byName assignation before calling onRun(), so that a CB that reSchedule the same name will work properly
--	 - Updated Version number to 2.14
--	2008-05-30 (Kiki):
--	 - Added function Chronos.getScheduledByName, that returns pairs(ChronosData.byName)
--	 - Updated Version number to 2.13
--	2008-05-29 (Kiki):
--	 - Removed remaining references to 'sea' library
--	 - Updated ToC to 20400
--	 - Updated Version number to 2.12
--]]

local CHRONOS_REV = 2.17;
...
if (not isBetterInstanceLoaded) then
	...
	function Chronos.schedule(when, handler, ...)
		...
		-- end
		-- if ( not when ) then 
		-- 	Chronos.printDebugError("CHRONOS_DEBUG_WARNINGS", "Chronos Error Detection: ", id , " has sent no interval for self function. ", when );
		-- 	return;
		-- end
	...
	function Chronos.isScheduledByName(name)
		...
		if ( not name ) then 
			Chronos.printError("No name specified to Chronos.isScheduledByName");
			return;
		end
		local namedSchedule = ChronosData.byName[name];
  ...
  function Chronos.getScheduledByName()
    return pairs(ChronosData.byName);
  end
	...
	function Chronos.startTimer( id ) 
		...
		-- Create a table for self id's timers
	...
	function Chronos.OnUpdate_Quick(self,elapsed)
		if ( not Chronos.online ) then 
			return;
		end
		...
		if ( ChronosData.elapsedTime ) then
			ChronosData.elapsedTime = ChronosData.elapsedTime + elapsed;
		else
			ChronosData.elapsedTime = elapsed;
		end
	...
	function Chronos.OnUpdate_Debug(self,elapsed)
		if ( not Chronos.online ) then 
			return;
		end
		if ( not ChronosData.variablesLoaded ) then 
