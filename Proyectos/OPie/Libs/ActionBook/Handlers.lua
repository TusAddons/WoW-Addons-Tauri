...
do -- extrabutton
	...
	local function extrabuttonHint()
		...
		if charges and maxCharges and charges < maxCharges and cdLeft == 0 then
			-- "time" here used to resolve to the WoW API global function
			-- time() (epoch seconds), not a number: unlike the sibling
			-- spellHint() above (which does "local time = GetTime()"),
			-- this scope never declares a local "time". That threw
			-- "attempt to perform arithmetic on a global 'time' (a
			-- function value)" and aborted the Extra Action Button hint
			-- whenever that action had charges below its max and was off
			-- cooldown.
			cdLeft, cdLength = chargeStart - GetTime() + chargeDuration, chargeDuration
		end
