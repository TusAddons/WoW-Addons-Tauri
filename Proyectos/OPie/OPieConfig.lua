...
do -- ext.config.bind
	...
	local alternateFrame = CreateFrame("Frame", nil, UIParent) do
		-- Only ever shown/positioned reactively via ToggleAlternateEditor
		-- (Show()+SetPoint() on demand); without an explicit initial
		-- Hide() it rendered at its default screen position as soon as
		-- the addon loaded.
		alternateFrame:Hide()
...
do -- ext.config.overlay
	...
	local promptFrame = CreateFrame("FRAME") do
		-- Only ever shown/parented reactively via config.prompt() ->
		-- config.overlay(); without an explicit initial Hide() it rendered
		-- as a blank 400x130 frame as soon as the addon loaded.
		promptFrame:Hide()
